...
import flashandmath.as3.parsers.CompiledObject;
 ...
 public class MathParser {
  ...
  protected  var f1Array:Array=["sin","cos","tan","ln","sqrt", "abs", "acos","asin", "atan" ,"ceil", "floor", "round"];
...
protected function sin(a){
	
	return Math.sin(a);
	
}

protected function cos(a){
...
protected function asin(a){
	
	return Math.asin(a);
	
}

protected function acos(a){
...
protected function ceil(a){
	
	return Math.ceil(a);
	
}

protected function round(a){
	...
	return operands[0]
		...
		public function doDeriv(compiledExpression:Array,aVarVals:Array):Number {
			...
			for(i = 0; i < compiledExpression.length; i++){
				...
				if(entrytype == "n"){
					operands.push( [ compiledExpression[i], 0 ]);
				} 
				else if (entrytype == "v"){
					if(compiledExpression[i]=="e"){
						operands.push( [ Math.E, 0 ] );
						
					} 
					else if(compiledExpression[i]=="pi"){
						operands.push( [ Math.PI, 0 ] );
					}
	
				  else {
					  for(j=0; j<aVarNames.length; j++){
						  if(compiledExpression[i]==aVarNames[j]){
							  operands.push( [ aVarVals[j], 1 ] );
						  }
					  }
				  }
				}
					...
					arg0=operands.pop();
					if(compiledExpression[i]==sin){
						operands.push( [ Math.sin(arg0[0]), Math.cos(arg0[0])*arg0[1] ] );
					} 
					
					else if(compiledExpression[i]==cos){
						operands.push( [ Math.cos(arg0[0]), -Math.sin(arg0[0])*arg0[1] ] );
					} 
					
					else if(compiledExpression[i]==tan){
						operands.push( [ Math.tan(arg0[0]), arg0[1]/(Math.cos(arg0[0])*Math.cos(arg0[0])) ] );
					}
					
					else if(compiledExpression[i]==asin){
						operands.push( [ Math.asin(arg0[0]), arg0[1]/Math.cos(Math.asin(arg0[0])) ] );
					}
					
					else if(compiledExpression[i]==acos){
						operands.push( [ Math.acos(arg0[0]), -arg0[1]/Math.sin(Math.acos(arg0[0])) ] );
					}
					
					else if(compiledExpression[i]==atan){
						operands.push( [ Math.atan(arg0[0]), arg0[1]*Math.cos(Math.atan(arg0[0]))*Math.cos(Math.atan(arg0[0])) ] );
					}
					
					else if(compiledExpression[i]==ln){
						operands.push( [ Math.log(arg0[0]), arg0[1]/arg0[0] ] );
					}
					
					else if(compiledExpression[i]==sqrt){
						operands.push( [ Math.sqrt(arg0[0]), arg0[1]/(2*Math.sqrt(arg0[0])) ] );
					}
					
					else if(compiledExpression[i]==abs){
						operands.push( [ Math.abs(arg0[0]), NaN ] );
					}
					
					else if(compiledExpression[i]==ceil){
						operands.push( [ Math.ceil(arg0[0]), NaN ] );
					}
					
					else if(compiledExpression[i]==floor){
						operands.push( [ Math.floor(arg0[0]), NaN ] );
					}
					
					else if(compiledExpression[i]==round){
						operands.push( [ Math.round(arg0[0]), NaN ] );
					}
					...
					if(compiledExpression[i]==mul){
						operands.push( [ arg1[0]*arg2[0], arg1[1]*arg2[0] + arg1[0]*arg2[1] ] );
					} 
					
					else if(compiledExpression[i]==plus){
						operands.push( [ arg1[0]+arg2[0], arg1[1]+arg2[1] ] );
					}
						...
						operands.push( [ arg1[0]/arg2[0], (arg2[0]*arg1[1]-arg2[1]*arg1[0])/(arg2[0]*arg2[0]) ] );
					}
					
					else if(compiledExpression[i]==pow){
						if (arg2[1] == 0) {
							operands.push( [ Math.pow(arg1[0],arg2[0]), Math.pow(arg1[0],arg2[0]-1)*arg2[0]*arg1[1] ] );
						}
						else {
							operands.push( [ Math.pow(arg1[0],arg2[0]), Math.pow(arg1[0],arg2[0]-1)*(arg2[0]*arg1[1] + arg1[0]*arg2[1]*Math.log(arg1[0]) ) ] );
						}
					}
	
					else if(compiledExpression[i]==min){
						operands.push( [ Math.min(arg1[0],arg2[0]), NaN ] );
					}
					
					else if(compiledExpression[i]==max){
						operands.push( [ Math.max(arg1[0],arg2[0]), NaN ] );
					}
